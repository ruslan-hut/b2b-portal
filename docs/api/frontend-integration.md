...
This document outlines the API endpoints relevant for the frontend application. For complete backend API documentation, see [Authentication and Common Patterns](./authentication-and-common-patterns.md).
...
|-------|------|----------|-------------|
| `uid` | string | Yes | Unique order identifier |
| `client_uid` | string | Yes | Client who placed the order |
| `store_uid` | string | Yes | Store for inventory allocation |
| `status` | string | Yes | Order status: `draft`, `new`, `processing`, `confirmed` |
| `total` | int | No | Total order amount in cents |
| `shipping_address` | string | No | Delivery address |
| `billing_address` | string | No | Invoice address |
| `comment` | string | No | Order notes |
| `discount` | int | No | Discount percentage applied |
| `vat_rate` | float | No | VAT rate applied |
| `vat_number` | string | No | Client's VAT number |
| `currency_code` | string | No | Currency code (e.g., "EUR") |
| `items` | array | No | Order items (see Order Item Fields below) |

#### Order Item Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `order_uid` | string | Yes | Parent order UID (composite key) |
| `product_uid` | string | Yes | Product UID (composite key) |
| `quantity` | int | Yes | Quantity ordered |
| `price` | int | Yes | Unit price in cents |
| `discount` | int | No | Discount amount in cents |
| `total` | int | No | Line total in cents |

**Note:** Order items use a composite key of (`order_uid`, `product_uid`). When upserting items with the same combination, the existing item is updated.
...
| `phone` | string | Phone number (8-15 characters) |
| `email` | string | Email address |
| `address` | string | Physical address |
| `discount` | int | Fixed discount percentage (0-100), used when `fixed_discount=true` |
| `additional_discount` | int | Bonus discount in percentage points added after any product discount limit |
| `fixed_discount` | bool | If true, use `discount` field; if false, calculate from discount scale |
| `cumulative_discount` | bool | If true, use `balance + order_total` for scale lookup; if false, use only `order_total` |
| `balance` | int | Current monthly purchase turnover in cents (set by CRM) |
| `vat_rate` | float | VAT rate percentage (0-100) |
| `vat_number` | string | VAT registration number |
| `business_registration_number` | string | Business registration number (required to confirm orders) |
| `price_type_uid` | string | Price type for this client |
| `store_uid` | string | Assigned store for inventory allocation |
| `active` | bool | Whether client can log in |

**Discount Calculation (three modes):**
1. **Fixed Discount** (`fixed_discount: true`): Use the `discount` field value directly
2. **Cumulative Scale-Based** (`fixed_discount: false`, `cumulative_discount: true`): Backend calculates discount from the discount scale based on `balance + order_total`. This allows clients to build up turnover over time to reach higher discount tiers.
3. **Per-Order Scale-Based** (`fixed_discount: false`, `cumulative_discount: false`): Backend calculates discount from the discount scale based on `order_total` only. Each order is evaluated independently without considering historical balance.

Whichever mode applies, the per-line discount is then resolved as
`min(earned_discount, product_limit) + additional_discount`, capped at 100%. The
additional discount is applied last and so goes over the product limit.
...
- [Authentication and Common Patterns](./authentication-and-common-patterns.md) - Complete API reference
- [Frontend Overview](../architecture/frontend-overview.md) - Frontend setup and usage
- [Frontend Coding Policy](../development/frontend-coding-policy.md) - Development patterns
